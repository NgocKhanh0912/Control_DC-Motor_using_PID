...
// Task này sẽ dừng (dừng motor) sau stop_time (giây)
void pid_task(void *pvParameter) 
{
    float output;
    unsigned long start_time = esp_timer_get_time() / 1000000; 

    while (pid_task_running) 
    { 
        unsigned long current_time = esp_timer_get_time() / 1000000; 
